...
PanelWindow {
    ...
    // --- caminho do wallpaper: passe via variável de ambiente ORZHOV_BACKGROUND
    // ou caia pro default embutido no próprio diretório do greeter ---------
    ...
    // --- estado da tela: "select" | "password" | "authenticating" --------
    ...
    // --- prompt atual pedido pelo greetd/PAM ------------------------------
    ...
    Connections {
        ...
        function onAuthMessage(message, error, responseRequired, echoResponse) {
            ...
            if (responseRequired) {
                ...
                // mensagem informativa (ex: aguardando leitor biométrico) -
                // precisa responder mesmo sem texto pra não travar o protocolo
                waitingResponse = false
                Greetd.respond("")
            }
        }
    ...
    // overlay escurecido (equivalente ao #screen:after do login.css:
    // color-mix(grey_300 32%) + blur). Sem backdrop-filter nativo em
    // QtQuick, aproximamos com um tint semi-opaco sobre o wallpaper.
    Rectangle {
        anchors.fill: parent
    ...
    // --- centro: seleção de usuário OU formulário de senha ----------------
    Item {
        anchors.centerIn: parent
        width: parent.width
        height: 320

        // grade de usuários (Card reaproveitado)
        Row {
            id: userGrid
            anchors.centerIn: parent
            spacing: 24
        ...
        // formulário de senha / prompt do PAM
        Column {
            id: loginForm
            anchors.centerIn: parent
            spacing: 24
            ...
            function triggerShake() { shakeAnim.start() }
            ...
            Button {
                id: loginButton
                function trigger() { window.submitResponse(passwordInput.text) }
    ...
    // --- canto inferior esquerdo: sessão -----------------------------------
    Row {
    ...
    // --- canto inferior direito: energia (loginctl / systemctl) -----------
    Row {
    ...
    Process {
        id: powerProc
        function exec(cmd) { command = cmd; running = true }
    }
}
